...
const sheetVariants = cva(
  "glass-strong fixed z-50 flex flex-col gap-4 shadow-2xl transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
...
function SheetContent({
  ...
  return (
    <SheetPortal>
      <SheetOverlay />
      <SheetPrimitive.Content className={cn(sheetVariants({ side }), "p-5", className)} {...props}>
        {children}
        <SheetPrimitive.Close className="absolute top-4 right-4 rounded-lg p-1 text-muted-foreground opacity-70 outline-none transition-opacity hover:opacity-100 hover:bg-secondary">
          <X className="size-4" />

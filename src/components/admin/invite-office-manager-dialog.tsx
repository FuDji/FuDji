...
import { inviteOfficeManager, type ActionState } from "@/app/admin/actions";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function InviteOfficeManagerDialog({ companyId }: { companyId: string }) {
  const [open, setOpen] = useState(false);
  const [state, formAction] = useActionState<ActionState, FormData>(inviteOfficeManager, undefined);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => (setOpen(v), setCopied(false))}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Pozovi office menadžera</DialogTitle>
          <DialogDescription>Kreiraj pozivnicu za administratora ove firme.</DialogDescription>
        </DialogHeader>
        {state?.inviteLink ? (
